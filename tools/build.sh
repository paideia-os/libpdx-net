...
#   4. paideia-as on $PATH (must be >= MIN_VERSION)
#
# Wave N seed (libpdx-net.M3-M4 scaffold): sources compile per-file with
# `paideia-as build --emit elf64`; there is no cross-file link resolution
# in this build step. Undefined externals across files are a link-time
# concern that surfaces only when a consumer tool statically links these
# objects; the per-file object-emit step is the discipline this loop
# enforces (matches libpdx-elevate + libpdx-audit convention).
